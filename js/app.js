document.addEventListener("DOMContentLoaded", () => {
    ...
    musicButton.addEventListener("click", () => {
        isMusicOnly = true;
        performSearch(apiKeyInput.value.trim());
    });
    ...
    function performSearch(query) {
        ...
        let url;
        if (isMusicOnly) {
            url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCategoryId=10&maxResults=20&q=${encodeURIComponent(query)}&key=${apiKey}`;
        } else {
            url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=20&q=${encodeURIComponent(query)}&key=${apiKey}`;
        }
