...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Fetch data from Cobalt API for watching or downloading the video
    function fetchCobaltApi(videoId) {
        const cobaltApiUrl = `https://your-cobalt-api-url.com/videos/${videoId}`;
        // Example fetch request to Cobalt API
        fetch(cobaltApiUrl)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                // Handle the response from Cobalt API (e.g., display video player)
                console.log(data);
                // Example: Update player container with video player
            ...
            .catch(error => {
                console.error('Error fetching data from Cobalt API:', error);
                alert('Failed to fetch video from Cobalt API');
            });
    }
